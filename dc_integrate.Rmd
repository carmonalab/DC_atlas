...
```{r}
DimPlot(ref, group.by = "functional.cluster", label = TRUE) + theme(aspect.ratio = 1)
ggsave("plots/DC_reference_wTcells.pdf", height=5, width=7)
```

```{r}
p1 <- DimPlot(ref, reduction="umap", group.by="Study") + theme(aspect.ratio = 1)
p2 <- DimPlot(ref, reduction="umap", group.by="scGate_multi", cols=palette) + theme(aspect.ratio = 1)
p1 | p2
...
heat <- DoHeatmap(ref.copy, assay="RNA", features=genes.use, group.by = "functional.cluster")
...
```{r fig.height=3}
FeaturePlot(ref.copy, features=c("AXL","SIGLEC6","SIGLEC1"), order=T)
```
